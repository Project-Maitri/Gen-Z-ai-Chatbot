...
for (const [lang, keys] of Object.entries(newKeys)) {
  const regexLabel = new RegExp('(\\\\b' + lang + ':\\\\s*{[\\\\s\\\\S]*?userNameLabel:\\\\s*")[^"]*(",)', 'g');
  const regexPlaceholder = new RegExp('(\\\\b' + lang + ':\\\\s*{[\\\\s\\\\S]*?userNamePlaceholder:\\\\s*")[^"]*(",)', 'g');
  
  content = content.replace(regexLabel, '$1' + keys.userNameLabel + '$2');
  content = content.replace(regexPlaceholder, '$1' + keys.userNamePlaceholder + '$2');
}
